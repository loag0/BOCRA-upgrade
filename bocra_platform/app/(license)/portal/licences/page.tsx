...
const MOCK_LICENCES = [
  {
    ...
    issued: "2024-03-01",
    expires: "2027-02-28",
    conditions: ["Must not exceed 500 subscribers without upgrading to NFP", "Annual QoS report due 31 March"],
  },
  {
    ...
    issued: "2023-08-15",
    expires: "2026-08-14",
    conditions: ["30% local content quota — monitored quarterly", "Retransmission licence required for relay signals"],
  },
  {
...
const LICENCE_STATUS: Record<string, { label: string; icon: React.ElementType; color: string; bg: string }> = {
  active:        { label: "Active",        icon: CheckCircle2, color: "text-bocra-green", bg: "bg-green-50" },
  expiring_soon: { label: "Expiring Soon", icon: AlertCircle,  color: "text-amber-600",   bg: "bg-amber-50" },
  expired:       { label: "Expired",       icon: XCircle,      color: "text-bocra-red",    bg: "bg-red-50" },
...
const APP_STATUS: Record<string, { label: string; color: string; bg: string }> = {
  ...
  submitted:    { label: "Submitted",      color: "text-bocra-blue", bg: "bg-blue-50" },
  under_review: { label: "Under Review",   color: "text-amber-600",  bg: "bg-amber-50" },
  approved:     { label: "Approved",       color: "text-bocra-green",bg: "bg-green-50" },
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-heading text-2xl font-bold text-bocra-navy">My Licences</h1>
          <p className="text-sm text-gray-500 mt-0.5">All BOCRA licences held by your organisation</p>
        </div>
        <Link
          href="/portal/apply"
      ...
      <section>
        ...
        {activeLicences.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
            ...
            <p className="text-gray-400 text-sm">No active licences on your account.</p>
            <Link href="/portal/apply" className="mt-3 inline-block text-sm text-bocra-blue hover:underline">
          ...
          <div className="space-y-4">
            {activeLicences.map((lic) => {
              ...
              return (
                <div key={lic.ref} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  <div className="p-5">
                    <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-semibold text-bocra-navy">{lic.category}</p>
                        <p className="text-xs text-gray-400 mt-0.5">
                          {lic.org} · Issued {formatDate(lic.issued)} · Expires {formatDate(lic.expires)}
                        </p>

                        {lic.conditions.length > 0 && (
                          <div className="mt-3 space-y-1">
                            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Licence conditions</p>
                            {lic.conditions.map((cond, i) => (
                              <div key={i} className="flex items-start gap-1.5 text-xs text-gray-500">
                                <span className="text-bocra-blue mt-0.5">·</span>
                                {cond}
                              </div>
                            ))}
                          </div>
                      ...
                      <div className="flex flex-col gap-2 flex-shrink-0">
                        <button
                          onClick={() => toast.info("Your licence certificate is being prepared for download.")}
                        ...
                        {statusKey === "expiring_soon" && (
                          <button
                            onClick={() => toast.success("A renewal application has been started. You'll be redirected shortly.")}
      ...
      <section>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm divide-y divide-gray-50">
          {MOCK_APPLICATIONS.map((app) => {
            ...
            return (
              <div key={app.ref} className="p-4 flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                  ...
                  <p className="text-sm font-medium text-bocra-navy">{app.type}</p>
                  <p className="text-xs text-gray-400 mt-0.5">
                    Submitted {formatDate(app.submitted)} · Officer: {app.officer}
                  </p>
                  {app.notes && (
                    <p className="text-xs text-gray-500 mt-1.5 bg-gray-50 rounded-lg px-3 py-1.5">{app.notes}</p>
                  )}
                </div>
            ...
            {expiredLicences.map((lic) => (
              <div key={lic.ref} className="p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">Expired {formatDate(lic.expires)}</p>
                </div>
                <span className="text-xs bg-gray-100 text-gray-400 px-2 py-0.5 rounded-full">Expired</span>
              </div>
            ))}
          </div>
        </section>
      ...
      <div className="bg-bocra-navy/5 rounded-xl px-5 py-4 flex items-center gap-4">
        <Building2 className="w-5 h-5 text-bocra-navy flex-shrink-0" />
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-bocra-navy">Need help with your licences?</p>
          <p className="text-xs text-gray-500 mt-0.5">
            Contact the Licensing Division at{" "}
            <a href="mailto:licensing@bocra.org.bw" className="text-bocra-blue hover:underline">
